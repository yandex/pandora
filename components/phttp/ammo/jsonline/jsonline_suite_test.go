...
func newTestProvider(conf Config) core.Provider {
...
func Benchmark(b *testing.B) {
	...
	Expect(err).To(BeNil())
	pool := sync.Pool{
		New: func() interface{} { return &ammo.Simple{} },
	}
	b.Run("Decode", func(b *testing.B) {
		for n := 0; n < b.N; n++ {
			decoder{}.Decode(jsonDoc, &ammo.Simple{})
		}
	})
	b.Run("DecodeWithPool", func(b *testing.B) {
		for n := 0; n < b.N; n++ {
			h := pool.Get().(*ammo.Simple)
