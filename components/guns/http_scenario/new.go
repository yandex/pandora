...
func NewHTTPGun(conf phttp.GunConfig, answLog *answlog.Logger) *ScenarioGun {
	return newScenarioGun(phttp.HTTP1ClientConstructor, conf, answLog)
}

// NewHTTP2Gun return simple HTTP/2 gun that can shoot sequentially through one connection.
func NewHTTP2Gun(conf phttp.GunConfig, answLog *answlog.Logger) (*ScenarioGun, error) {
	if !conf.SSL {
...
func newScenarioGun(clientConstructor phttp.ClientConstructor, cfg phttp.GunConfig, answLog *answlog.Logger) *ScenarioGun {
	return &ScenarioGun{
