...
Где:
   ...
   * `call` - полное имя сервиса и его вызываемый метод. В качестве разделителя сервиса и метода может использоваться `.` или `/`. Например, `api.Adder.Add`, `api.Adder/Add` или `/api.Adder/Add`.
